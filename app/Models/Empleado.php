...
class Empleado extends Model
{
    ...
    protected $primaryKey = 'idEmplaedo';
    ...
    /**
     * Obtiene los datos de la tabla puesto
     *
     * @author Santa Garcia
     *
     * @return string
     */
    public function obtenDatosPuesto ($condicion = 0)
    {
        settype($idEmpleado, 'integer');

        $this->db->select('idPuesto');
        $this->db->from(TBL_PUESTO);
        $this->db->like('descripcion', $condicion, 'both');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
    ...
    /**
     * Obtiene el nombre del gerente general de la unidad de negocio
     *
     * @param integer $club IdUn
     *
     * @author Antonio Sixtos
     *
     * @return string
     */
    public function obtenGerenteGeneral($club)
    {
        $query = $this->db->query(
            'select concat(p.nombre," ",p.paterno," ",p.materno) as nombreCompleto from '.TBL_PERSONA.' p '.
            'left join '.TBL_EMPLEADO.' e on e.idPersona=p.idPersona '.
            'left join '.TBL_EMPLEADOPUESTO.' ep on ep.idEmpleado=e.idEmpleado '.
            'left join '.TBL_UN.' u on u.idUn=ep.idUn '.
            'where ep.idPuesto IN (74,100) and e.idTipoEstatusempleado='.ESTATUS_EMPLEADO_ACTIVO.' and ep.idUn='.$club." ORDER BY idPuesto limit 1"
        );
        $row = $query->row();
        $nombreGG = $row->nombreCompleto;
        return $nombreGG;
    }
    ...
    /**
     * Regresa el idPersona buscando con el idEmpleado
     *
     * @param integer $idEmpleado Id Empleado a Filtrar
     *
     * @author Jonathan Alcantara
     *
     * @return integer
     */
    public function obtenIdPersona ($idEmpleado = 0)
    {
        $idPersona = 0;
        ...
        $sql = "SELECT idPersona FROM ".TBL_EMPLEADO." WHERE idEmpleado = ".$idEmpleado.";";
        $query = $this->db->query($sql);
        $idPersona = ($query->num_rows > 0) ? $query->row_object()->idPersona : '';

        return $idPersona;
    }

    /**
     * Obtiene cual es la pagina de inicio seleccionada en base al puesto del empleado
     *
     * @param integer $puesto Identificador de puesto
     *
     * @author Jorge Cruz
     *
     * @return string
     */
    public function obtenPaginaInicioPuesto($puesto)
    {
        settype($puesto, 'integer');
        ...
        $this->db->select('paginaInicio');
        $this->db->from(TBL_PUESTO);
        $this->db->where('idPuesto', $puesto);
        $query = $this->db->get();

        if (trim($query->row_object()->paginaInicio) == 'agendaAnfitrion') {
            $ci =& get_instance();
            $ci->load->model('checkup_model');
            $ci->load->model('permisos_model');

            $datos['club'] = $this->session->userdata('idUn');
            $listaConsultorio = $ci->checkup_model->listaConsultorio($datos['club'],0,1);
            $contador = 0;
            foreach ($listaConsultorio as $key => $value) {
                if ($value == 'Afiliacion') {
                    $contador++;
                    break;
                }
            ...
            if ($contador == 1) {
                return 'agendaAnfitrion' ;
            } else {
                return null;
            }
        } else {
            return $query->row_object()->paginaInicio;
        }
    }

    /**
     * Regresa lista de puestos
     *
     * @author Jonathan Alcantara
     *
     * @return array
     */
    public function obtenPuestos ()
    {
        $datos = array();

        $this->db->where('descripcion <> ""');
        $this->db->where('descripcion <> "00000"');
        $this->db->where('fechaEliminacion', '0000-00-00 00:00:00');
        $query = $this->db->select(
            "idPuesto, codigo, descripcion AS puesto, paginaInicio",
            false
        )->order_by('puesto')->get(TBL_PUESTO);
     ...
     * @return array
     */
    public function obtieneOperador ($idOperador)
    {
        settype($idOperador, 'integer');
        $razonSocial = '';

        $where = array('idOperador' => $idOperador, 'fechaEliminacion' => '0000-00-00 00:00:00');
        $query = $this->db->select('razonSocial')->get_where(TBL_OPERADOR, $where);
        if ($query->num_rows > 0) {
            $razonSocial = $query->row()->razonSocial;
        }
        return $razonSocial;
    }

    /**
     * Obtiene Nombre puesto
     *
     *
     * @return array
     */
    public function obtienePuesto ($idPuesto)
    {
        settype($idPuesto, 'integer');
        $nombre = '';
        ...
        $where = array('idPuesto' => $idPuesto, 'fechaEliminacion' => '0000-00-00 00:00:00');
        $query = $this->db->select('descripcion')->get_where(TBL_PUESTO, $where);
        if ($query->num_rows > 0) {
            $nombre = $query->row()->descripcion;
        }
        return $nombre;
    }

    /**
     * Valida si un empleado puede dar una actividad deportiva
     *
     * @param integer $idEmpleado           Identificador de empleado
     * @param integer $idActividadDeportiva Identificador de actividad deportiva
     *
     * @author Jonathan Alcantara
     *
     * @return array
     */
    public function validaActividadDeportiva ($idEmpleado, $idActividadDeportiva)
    {
        settype($idEmpleado, 'integer');
        settype($idActividadDeportiva, 'integer');
        ...
        $datos['error']               = 1;
        $datos['mensaje']             = 'Faltan datos';
        $datos['idEmpleadoActividad'] = 0;
        $datos['activo']              = 0;

        if ( ! $idEmpleado or ! $idActividadDeportiva) {
            return $datos;
        }
        $datos['error']   = 0;
        $datos['mensaje'] = '';
        ...
        $query = $this->db->select('idEmpleadoActividad, activo')->get_where(TBL_EMPLEADOACTIVIDAD, $where);

        if ($query->num_rows) {
            $row = $query->row();
            $datos['idEmpleadoActividad'] = $row->idEmpleadoActividad;
     ...
     * @return string
     */
    public function validaDatosEmpleado($usuario, $numEmpleado, $numImss=0)
    {
        settype($numEmpleado, 'integer');

        $this->db->select('e.idEmpleado');
        $this->db->from(TBL_EMPLEADO.' e');
        $this->db->join(TBL_USUARIOS.' u', 'u.idEmpleado = e.idEmpleado');
        $where = array('e.idEmpleado' => $numEmpleado, 'u.fechaEliminacion'=>'0000-00-00 00:00:00', 'e.fechaEliminacion'=>'0000-00-00 00:00:00','e.idTipoEstatusEmpleado'=>ESTATUS_EMPLEADO_ACTIVO,'u.NombreUsuario'=>$usuario);
        if ($numImss > 0) {
            $this->db->where('imss',$numImss);
        }
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $fila) {
               return $fila->idEmpleado;
            }
        } else {
            return 0;
        }
     ...
     * @return string
     */
    public function validaEmpleado($idEmpleado)
    {
        settype($idEmpleado, 'integer');

        $this->db->select('idEmpleado');
        $this->db->from(TBL_EMPLEADO);
        $where = array('idEmpleado'=>$idEmpleado,'idTipoEstatusEmpleado' => ESTATUS_EMPLEADO_ACTIVO,'fechaEliminacion'=>'0000-00-00 00:00:00');
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $fila) {
    ...
    /**
     * Valida la existencia del imss del  empleado
     *
     * @author Santa Garcia
     *
     * @return string
     */
    public function validaImss($imss)
    {
        $this->db->select('idEmpleado');
        $this->db->from(TBL_EMPLEADO);
        $where = array('imss' => $imss,'fechaEliminacion'=>'0000-00-00 00:00:00');
        $this->db->where($where);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $fila) {
